...
|---|---|---|---|
| 会議室 | 利用枠の重なり判定と「会議室を一時的に利用不能とする場合」の扱いは会議室ごとに決まるが、正本は会議室を所与としてしか扱っておらず、利用不能の決まりを置ける概念が無い。モデルでは会議室を予約の識別子（値）としてしか参照できず、利用不能の操作をどの要素にも置けない | 業務ルール「仮押さえ予約と確定予約」の重なり判定、BDD-013、BDD-014、まだ決まっていないこと「会議室を一時的に利用不能とする場合」 | 概念 |
| 繰上げの不成立 | 予約待ちが繰り上がったのに仮押さえ予約が成立しなかった場面が正本に無く、モデルでは予約待ちを戻すのか次を繰り上げるのかを決められない | 業務ルール「予約待ち」の繰上げ、BDD-007 | 業務イベント、BDD |
