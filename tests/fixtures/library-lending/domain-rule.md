...
本を返す約束の日で、貸出日時の日の14日後である。貸したときに決まり、この日を過ぎても返されなければ延滞になる。
...
借りられるのは、延滞の貸出が一つも無く、借りている冊数が貸出上限の5冊未満で、その本がほかの貸出中か延滞の貸出に属していないときである。4冊借りていれば5冊目は借りられ、5冊借りていれば6冊目は借りられない。借りると、その日時を貸出日時、その日の14日後を返却期限とする貸出中の貸出が生まれ、「本が貸し出された」が起きる。延滞の貸出があれば「延滞の貸出がある利用者が本を借りる」、借りている冊数が貸出上限に達していれば「貸出上限に達している利用者が本を借りる」、本がほかに貸出中なら「貸出中の本を借りる」として拒む。
...
判定日時の日が返却期限の翌日以降で、貸出がまだ貸出中なら、延滞にする。返却期限の当日はまだ延滞にしない。延滞にすると「貸出が延滞になった」が起きる。判定日時の日が返却期限の当日以前なら、約束の期間内の利用者を貸出停止にしてしまうので、「返却期限を過ぎていない貸出を延滞にする」として拒む。
...
返却期限は、貸出日時の日に14日を足した日である。借りている冊数は、その利用者の貸出中と延滞の貸出の数である。
...
貸出の延長を認めるかは決まっていない。認めるなら返却期限が変わるので、「返却期限は貸出日時の日の14日後で、貸出の間変わらない」を見直す。
