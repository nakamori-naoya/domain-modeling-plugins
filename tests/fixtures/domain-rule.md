...
組織内の共用会議室を予約する業務について、予約者と予約業務責任者が同じ判断を説明するための業務知識である。会議室の仮押さえ、確定、取消、予約待ち、無断不利用による予約資格の制限を扱い、予約業務責任者と会議室運用責任者が業務の正誤を判断した。この資料は架空の題材によるdomain-rule型のfixtureであり、用語、境界、不変条件、状態、誰が行えるか、競合、拒否理由を業務判断に使える粒度で記載している。
...
**この業務が担わないこと**: 会議室そのものの登録・改廃と設備管理は施設管理業務が担い、この業務は登録済みの会議室を所与として扱う。利用料金の算定と請求は料金業務が担う。会議室の入退室記録を取ること自体は入退室管理業務が担い、この業務はその記録を無断不利用の判断材料として受け取る。顧客の利用登録と退会は顧客管理業務が担う。
...
| 仮押さえ期限 | 仮押さえ予約を確定できる期限 | 成立時から15分後とし、期限と同時の確定では期限到来を先に扱う |
...
| 顧客の利用登録が完了した | 顧客管理業務で利用登録が完了し、予約を申し込める顧客になった | 予約可能顧客として予約資格の判断が始まる |
...
|---|---|---|
| 空いている利用枠を探す | クエリ | なし |
| 利用枠を仮押さえする | コマンド | 仮押さえ予約が成立した |
| 仮押さえ予約を確定する | コマンド | 予約が確定した |
| 予約を取り消す | コマンド | 予約が取り消された |
| 予約待ちを登録する | コマンド | 予約待ちが登録された |
| 予約待ちを取り消す | コマンド | 予約待ちが取り消された |
| 自分の予約と予約待ちを見る | クエリ | なし |
| 予約待ちの順序を変更する | コマンド | 対象外（誰も行えない。受付順は登録時に決まる） |
...
同じ利用枠に複数の予約待ちがある場合、受付日時が最も早い予約待ちを繰り上げる。受付日時も同じ場合は予約待ち番号が小さい方を先にする。
...
| 引き金（起きること） | 何が移るか |
|---|---|
| 仮押さえ予約が成立した | 予約が生まれ、仮押さえ予約になる |
| 予約が確定した | 仮押さえ予約が確定予約へ移る |
| 予約が取り消された | 仮押さえ予約または確定予約が取消済み予約へ移る |
| 仮押さえ期限が到来した | 未確定の仮押さえ予約が期限切れ予約へ移る |
| 予約待ちが登録された | 予約待ちが生まれ、予約待ち中になる |
| 予約待ちが取り消された | 予約待ち中が取消済み予約待ちへ移る |
| 予約待ちから仮押さえ予約が成立した | 予約待ち中が繰上済み予約待ちへ移り、同時に予約が生まれる |
| 顧客の利用登録が完了した | 顧客が予約可能顧客になる |
| 仮押さえ停止期間が始まった | 予約可能顧客が仮押さえ停止中顧客へ移る |
| 仮押さえ停止期間が終了した | 仮押さえ停止中顧客が予約可能顧客へ移る |
...
|---|---|---|---|---|
| 空いている利用枠を探す | クエリ | 利用登録済みの顧客 | 登録していない人は予約を申し込めないため、探す意味を持たない | 認証・認可 |
| 利用枠を仮押さえする | コマンド | 予約可能顧客である予約者 | 仮押さえ停止中顧客は新しい利用枠を確保できない | 業務判断 |
| 仮押さえ予約を確定する | コマンド | その予約の予約者本人のみ | 第三者は本人の利用意思を示せない | 認証・認可 |
| 予約を取り消す | コマンド | その予約の予約者本人のみ | 第三者は本人の利用約束を終了できない | 認証・認可 |
| 予約待ちを登録する | コマンド | 予約可能顧客である予約者 | 仮押さえ停止中顧客は新しい利用希望を追加できない | 業務判断 |
| 予約待ちを取り消す | コマンド | その予約待ちの予約者本人のみ | 第三者は本人の利用希望を終了できない | 認証・認可 |
| 自分の予約と予約待ちを見る | クエリ | その予約・予約待ちの予約者本人のみ | 他人の利用予定は業務上知る必要がない | 認証・認可 |
| 予約待ちの順序を変更する | コマンド | 誰も行えない | 受付順を人が変えると公平性を説明できない | 業務判断 |
...
予約待ちの登録時に、繰上げ時の仮押さえを受け入れる意思を確認する。意思が確認できない申込みは予約待ちとして登録しない。

確定予約が取り消されたとき、同じ利用枠の予約待ち中から受付順が最も早い一件を選び、15分間の仮押さえ予約を成立させる。繰上げから生まれた仮押さえ予約が期限切れになった場合も、次の予約待ちを同じ方法で繰り上げる。
...
ここまでの決まりが、具体の場面でどう働くかを示す。各シナリオは、それより前のシナリオの結果に依存しない。

## 仮押さえ予約の成立と確定

| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-001 | 予約可能顧客、利用枠に予約なし | 仮押さえする | 仮押さえ予約が成立し、期限が15分後に決まる |
| BDD-002 | 仮押さえ予約、期限より前 | 確定する | 確定予約になり、利用枠は変わらない |
| BDD-013 | 隣接する利用枠に確定予約 | 仮押さえする | 重ならないので成立する |
| BDD-014 | 一部重なる利用枠に確定予約 | 仮押さえする | 成立しない |
| BDD-015 | 仮押さえ予約、期限と同時刻 | 確定する | 期限の到来が先に働き、確定しない |
| BDD-023 | 確定予約 | 再び確定する | 状態は変わらず、新しい確定予約も生まれない |

### [BDD-001] 空いている利用枠に仮押さえ予約が成立する
...
| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-003 | 確定予約、利用開始前、予約待ちなし | 予約者が取り消す | 取消済み予約になり、利用枠が空く |
| BDD-024 | 仮押さえ予約、利用開始前 | 予約者が取り消す | 取消済み予約になり、利用枠が空く |
| BDD-016 | 確定予約、別の予約者 | 別の予約者が取り消す | 取り消せず、確定予約は維持される |
| BDD-017 | 確定予約、利用開始と同時刻 | 予約者が取り消す | 取り消せず、確定予約は維持される |
| BDD-020 | 仮押さえ停止中顧客の確定予約 | 予約者が取り消す | 取消は成立し、停止状態は変わらない |

### [BDD-003] 利用開始前の確定予約を取り消すと利用枠が空く

```gherkin
Given: ある予約者の確定予約がある
  And: 利用枠は2026年9月18日 10:00から11:30までの大会議室である
  And: 2026年9月18日 10:00から11:30までの大会議室に予約待ち中はない
...
### [BDD-024] 利用開始前の仮押さえ予約を取り消すと利用枠が空く
...
Given: ある予約者の確定予約がある
  ...
  And: 別の予約者がいる
  And: 現在時刻は2026年9月18日 09:30である
When: 別の予約者がその確定予約を取り消す
Then: その確定予約は維持される
  And: 2026年9月18日 10:00から11:30までの大会議室の占有は続く
  NOTE: Rule: 自分の予約と予約待ちだけを取り消せる
    Reason: 取り消そうとしたのが予約者本人ではなく、本人の利用意思を第三者が変えようとしているため
```

### [BDD-017] 利用開始と同時刻の取消は成立しない
...
Then: その確定予約は維持される
  And: 2026年9月18日 10:00から11:30までの大会議室の占有は続く
  NOTE: Rule: 予約者は自分の予約を利用開始前まで取り消せる
    Reason: 取消の時刻が利用開始と同じで、利用開始前に含まれないため
```

### [BDD-020] 仮押さえ停止中でも既存の確定予約は取り消せる
...
| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-006 | 確定予約で埋まった利用枠 | 予約待ちを申し込む | 予約待ち中になり、受付日時が決まる |
| BDD-007 | 確定予約と、最も早い受付順の予約待ち | 確定予約を取り消す | 予約待ちが繰り上がり、仮押さえ予約が成立する |
| BDD-008 | 予約待ち中 | 予約者が取り消す | 取消済み予約待ちになり、繰上げ対象から外れる |
| BDD-018 | 空いている利用枠 | 予約待ちを申し込む | 登録されない |
| BDD-019 | 受付日時が同じ予約待ちが二件 | 確定予約を取り消す | 予約待ち番号が小さい方だけが繰り上がる |

### [BDD-006] 確定予約のある利用枠へ予約待ちが登録される

```gherkin
Given: 予約可能顧客である予約者がいる
  And: 2026年9月18日 10:00から11:30までの大会議室には別の予約者の確定予約がある
  And: その予約者は繰上げ時の仮押さえを受け入れる意思を示している
...
### [BDD-007] 確定予約の取消で最も早い予約待ちが繰り上がる

```gherkin
Given: 2026年9月18日 10:00から11:30までの大会議室に確定予約がある
  And: 同じ利用枠に別の予約者の予約待ちが予約待ち中である
  And: その予約待ちは同じ利用枠で受付順が最も早い
  And: その予約者は予約可能顧客である
When: 確定予約の予約者が自分の予約を2026年9月10日 12:00に取り消す
Then: 受付順が最も早い予約待ちは繰上済み予約待ちになる
  And: その予約者に2026年9月18日 10:00から11:30までの大会議室の仮押さえ予約が成立する
  And: 仮押さえ期限は2026年9月10日 12:15になる
```

### [BDD-008] 繰上げ前の予約待ちを予約者が取り消す
...
Then: その予約者の予約待ちは成立しない
  ...
  NOTE: Rule: 予約待ちは、同じ会議室と同じ利用枠に確定予約がある場合に登録できる
    Reason: 対象の利用枠に確定予約が無く、待つ対象が存在しないため
```

### [BDD-019] 受付日時が同じときは予約待ち番号が小さい方が繰り上がる

```gherkin
Given: 2026年9月18日 10:00から11:30までの大会議室に確定予約がある
  And: 同じ利用枠に予約待ちW-102とW-103があり、どちらも予約待ち中である
  And: W-102とW-103の受付日時はどちらも2026年9月2日 10:00である
  And: W-102とW-103より早い受付順の予約待ち中はない
  And: W-102とW-103の予約者はどちらも予約可能顧客である
When: 確定予約の予約者が2026年9月10日 12:00に自分の予約を取り消す
Then: 予約待ちW-102は繰上済み予約待ちになる
  And: W-102の予約者に2026年9月18日 10:00から11:30までの大会議室の仮押さえ予約が成立する
...
| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-009 | 直近30日で2回の無断不利用 | 3回目が記録される | 仮押さえ停止中顧客になり、既存の確定予約は残る |
| BDD-010 | 仮押さえ停止中顧客 | 仮押さえする | 成立しない |
| BDD-011 | 仮押さえ停止中顧客 | 停止期間が終了する | 予約可能顧客へ戻る |
| BDD-012 | 停止終了直後の予約可能顧客 | 仮押さえする | 成立する |

### [BDD-009] 3回目の無断不利用で仮押さえ停止期間が始まる
...
| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-005 | 未確定の仮押さえ予約 | 仮押さえ期限が到来する | 期限切れ予約になり、利用枠が空く |

### [BDD-005] 未確定のまま仮押さえ期限が到来する
...
| BDD | 主な事前状態 | 業務イベント | 業務結果 |
|---|---|---|---|
| BDD-004 | 空いている利用枠と二人の予約可能顧客 | 二人が同時に仮押さえする | 先に成立した一方だけが占有する |
| BDD-021 | 最も早い受付順の予約待ち中 | 取消が繰上げより先に成立する | 取消済み予約待ちになる |
| BDD-022 | 最も早い受付順の予約待ち中 | 繰上げが取消より先に成立する | 繰上済み予約待ちになる |

### [BDD-004] 同じ利用枠への同時仮押さえは一方だけ成立する
...
Then: 先に成立した一方だけに仮押さえ予約がある
  ...
  NOTE: Rule: 同じ会議室の重なる利用枠へ、現在有効な予約は一つしか存在しない
    Reason: 二件とも成立させると同じ利用枠に有効な予約が二つ生まれるため
```

### [BDD-021] 競合で予約待ちの取消が先に成立する
...
Then: その予約待ちは取消済み予約待ちになる
  ...
  NOTE: Rule: 一つの予約待ちは、繰上げまたは取消のどちらか一方で終わる
    Reason: 取消が先に成立して予約待ちが終わっており、同じ希望から予約を生まないため
```

### [BDD-022] 競合で予約待ちの繰上げが先に成立する
...
Then: その予約待ちは繰上済み予約待ちになる
  ...
  NOTE: Rule: 一つの予約待ちは、繰上げまたは取消のどちらか一方で終わる
    Reason: 繰上げが先に成立して予約待ちが終わっており、後から取消で終わらせないため
```
