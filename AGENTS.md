...
- 正式な定義を作る`bdd-discovery-and-formulation`は実行時の依存ではなく、その資料の絶対pathを入力として受け取るだけである。正式な定義の明示索引に無い語を要素にしない。新しい業務の事実が要ると分かったときに止まるか進むかは、`model-domain`のSKILL.mdの停止条件だけが決める。
