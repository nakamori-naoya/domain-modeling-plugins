...
目印: write-doc の domain-model 型の template が「検査が読む目印」に書いた形（クラス図と状態遷移図の Mermaid 記法、
  拒む理由の節の見出し、BDD番号）と、業務知識の資料（source.py の build_index が索引を導く）。
...
合格述語:
  ...
  - コマンド（+名前(引数)）は集約ルートとエンティティにだけあり、名前が業務知識の資料でコマンドとした行い、引数が同じ図のクラスのラベルである。
  ...
  - 業務知識の資料で状態を持つ集約ルートには、その集約の状態遷移図がある
  - 状態遷移図で矢印の出ていない状態があるコマンド（生成のコマンドを除く）は、その集約の節の中に、名前がそのコマンドの H3 を持つ（拒む理由の置き場）
  - 本文が引くBDD番号（「BDD-001〜006」の範囲を含む）が業務知識の資料にある
...
意味評価として残す範囲: 境界の引き方と集約の数、拒む理由の節が受け付けない状態のすべてを一文ずつ書いているか、文章が図の言い直しになっていないか、
  値オブジェクトの行が業務の語か、未決と業務知識への提案が要るものを漏らしていないか、warnings の本文が引いていないBDDが、本当にクエリか集約の外で成立するものか。

  verify.py --source <業務知識の資料の絶対path>  < <保存したドメインモデル資料>

exit 0 = 通った（stdoutに verified, source_path, warnings） / 2 = 標準入力が空、業務知識の資料が契約の節を持たない、または述語が成り立たない。
"""
...
def check(body: str, index: dict) -> tuple[list[str], list[str]]:
    ...
    for diagram in class_diagrams:
        ...
        for class_id, cls in classes.items():
            ...
            if label not in vocabulary:
                errors.append(f"クラス「{label}」は業務知識の資料の索引に無い語である。要素にせず業務知識への提案へ移す")
            ...
            for line in cls["body"]:
                ...
                if kind in holders:
                    ...
                    if name not in commands_in_source:
                        errors.append(f"クラス「{label}」のコマンド「{name}」は、業務知識の資料の業務の行いのコマンドに無い")
        ...
        for source_state, target_state, command, line in transitions:
            for state in (source_state, target_state):
                if state != "[*]" and state not in index["states"]:
                    errors.append(f"「{label}」の状態遷移図の状態「{state}」は業務知識の資料の状態に無い")
            if command is None:
                if target_state != "[*]":
    ...
    for label in roots:
        if label in index["state_holders"] and label not in section_of:
            errors.append(f"業務知識の資料で状態を持つ「{label}」の状態遷移図が無い")
            continue
    ...
    if unknown:
        errors.append("本文が引くBDD番号が業務知識の資料に無い: " + ", ".join(unknown))
    uncited = [ref for ref in index["bdd"] if ref not in cited]
    if uncited and section_of:
        warnings.append("本文が引いていない業務知識の資料のBDD（クエリか、集約の外で成立するものか、要素の不足かを読み返す）: " + ", ".join(uncited))
