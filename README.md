...
公開入口は`model-domain`の1つである。domain-ruleの正式な定義の絶対pathを渡すと新しい資料を作り、既存のドメインモデル資料のpathも渡すと、正式な定義の変更が及ぶ箇所だけを同じパスで更新する。正式な定義に無い語や決まりが要ると分かったら、図には使わず「業務知識への提案」に残し、正式な定義の反証（bddの`formulate-domain`）へ戻す。この入口は正式な定義を書き換えない。
