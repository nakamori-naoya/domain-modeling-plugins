...
2. **grill** — 正本と索引だけでは決まらない割り当て（集約の境界、状態ごとの型の分割、索引に無い語の扱い）のうち成果を左右する問いを順に渡して確かめる。対話の作法と問う数の上限は`grill`の公開契約に従い、問われなかった論点は推奨を仮置きして未決に載せる
3. **同一agentの判断** — 依頼・正本・索引・決定・未決を一つの文脈で読む
4. **割り当て** — 索引の語だけを要素にし、要素ごとに目的・何でないか・持つもの・不変条件・生成の条件・協働相手と、操作の契約を書く。正本に無い概念は要素にせず、`## 正本へ提案する概念`（名前・なぜ要るか・導いた業務ルール・BDD・足すべき節）へ分け、報告で`formulate-domain`（bdd）へ`references`として渡す次の一手を示す。この入口はdomain-ruleを書き換えない
5. **検査** — 候補本文を標準入力で渡し、正本pathから導いた索引に対して節と順序、明示索引に無い語、要素ごとの必須項目、操作の契約、BDDと要素の対応記載、正本へ提案する概念の表（提案した語が要素一覧に無い）を見る。通ったときに言えるのは構造述語が成り立ったことだけで、必要性や過不足は同じagentが意味判断する
